...
export function StatCard({ icon, value, label, trend, color = "gold", delay = 0, animate: doAnim = true }: StatCardProps) {
  const colors = colorMap[color];
  const isNumber = typeof value === "number";

  return (
    <motion.div
      initial={doAnim ? { opacity: 0, y: 24, scale: 0.95 } : undefined}
      animate={doAnim ? { opacity: 1, y: 0, scale: 1 } : undefined}
