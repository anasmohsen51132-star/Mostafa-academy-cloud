// prisma/seed.ts
import { PrismaClient, Role, AcademicLevel } from "@prisma/client";
...
async function main() {
  ...
  if (!existing) {
    ...
    await prisma.user.create({
      data: { name: ownerName, phone: ownerPhone, passwordHash: hash, role: Role.OWNER, avatar: "م" },
    });
  ...
  const course = await prisma.course.upsert({
    ...
    create: {
      ...
      levels: {
        create: [
          { academicLevel: AcademicLevel.FIRST_SECONDARY },
          { academicLevel: AcademicLevel.SECOND_SECONDARY },
          { academicLevel: AcademicLevel.THIRD_SECONDARY },
        ],
      },
    },
    update: {},
